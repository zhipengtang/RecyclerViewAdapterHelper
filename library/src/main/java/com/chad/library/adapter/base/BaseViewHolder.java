...
public class BaseViewHolder extends RecyclerView.ViewHolder {
     ...
     */
    private final SparseArray<View> views;//定义Views集合
   ...
   //获取单击是的Position
     ...
     */
    //设置View 上TextView的文本，第二个参数为设置文本的字符串
     ...
     * @return The BaseViewHolder for chaining.
     */
    //设置View 上ImageResourse,用于填充的图片通过第二个人参数传入
    ...
    public BaseViewHolder setAlpha(@IdRes int viewId, float value) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            ...
            alpha.setDuration(0);//设置延续时间为0
            alpha.setFillAfter(true);//设置动画结束后保存结束后的动画效果
            getView(viewId).startAnimation(alpha);//设置动画的透明度值
        }
        return this;//返回此ViewHolder
    }
    ...
    public BaseViewHolder addOnClickListener(@IdRes final int viewId) {
        ...
        if (view != null) {
            if (!view.isClickable()) {
                view.setClickable(true);//当View不为空且View还没有被点击，此时设置View为可点击的状态
            }
    ...
    public BaseViewHolder addOnLongClickListener(@IdRes final int viewId) {
        ...
        final View view = getView(viewId);
        if (view != null) {//当View不为空且View还没有被长按，此时设置View为可长按的状态
   ...
   //通过View 获得对应的View
    ...
    public <T extends View> T getView(@IdRes int viewId) {
        ...
        if (view == null) {
            ...
            views.put(viewId, view);//把View添加进去
        }
        return (T) view;
    }
