...
public class PullToRefreshAdapter extends BaseQuickAdapter<Status, BaseViewHolder> {
    ...
    * 设置Item使用的源图片
    * Item的Position对3求余
    * 如果刚好整除则Item的源图片设置为animation_img1
    * 如果为1则源图片设置为animation_img2
    * 如果为2则源图片设置为animation_img3*/
    @Override
    protected void convert(BaseViewHolder helper, Status item) {
        ...
        //设置Item的文本为Hoteis in Rio de Janeiro
        helper.setText(R.id.tweetName,"Hoteis in Rio de Janeiro");
        String msg="\"He was one of Australia's most of distinguished artistes, renowned for his portraits\"";
        ( (TextView)helper.getView(R.id.tweetText)).setText(SpannableStringUtils.getBuilder(msg).append("landscapes and nedes").setClickSpan(clickableSpan).create());
        ( (TextView)helper.getView(R.id.tweetText)).setMovementMethod(LinkMovementMethod.getInstance());
    }
    //在可点击的范围内单击用Toast打印提示信息，同时更新绘画状态
    ClickableSpan clickableSpan = new ClickableSpan() {
        @Override
        public void onClick(View widget) {
            ToastUtils.showShortToast("事件触发了 landscapes and nedes");//当点击图片时用Toast打印一段文字提示用户点击了哪里
        }

        @Override
        public void updateDrawState(TextPaint ds) {
            ds.setColor(Utils.getContext().getResources().getColor(R.color.clickspan_color));
            ds.setUnderlineText(true);//设置landscapes and nedes加入下划线
        }
    };
